...
| `-cdisk N`, `--cache-disk N` | `LLAMA_ARG_CACHE_DISK` | `0` | Maximum global NVMe/SSD disk quota in MiB for cold Radix checkpoint chunks shared across all model subdirectories. When the server boots, existing on-disk chunks are automatically scanned, validated, and hydrated into the Radix Tree topology for instant prefix reuse across process restarts. When exceeded, root LRU scavenging automatically evicts the globally oldest `.ckpt` files; `0` disables disk offloading; `-1` removes the quota (bound only by physical disk capacity). |
