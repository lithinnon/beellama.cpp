...
static bool parse_uint(const std::string & in, size_t & pos, uint32_t & out) {
...
static bool find_key(const std::string & in, size_t & pos, const std::string & key) {
    while (pos < in.size()) {
        while (pos < in.size() && (std::isspace((unsigned char) in[pos]) || in[pos] == ',' || in[pos] == '{' || in[pos] == '\n' || in[pos] == '\r')) pos++;
        if (pos >= in.size() || in[pos] != '"') return false;
        pos++;
        size_t start = pos;
...
bool load(matrix & m, const std::string & path) {
    ...
    uint32_t nl_u = 0;
    if (!find_key(s, pos, "nl") || !parse_uint(s, pos, nl_u)) return false;
    int nl = (int) nl_u;

    uint32_t ne_u = 0;
    if (!find_key(s, pos, "ne") || !parse_uint(s, pos, ne_u)) return false;
    int ne = (int) ne_u;
